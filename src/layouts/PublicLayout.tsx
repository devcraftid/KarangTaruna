...
import { Home, Trophy, Info, PieChart, User, LogIn } from 'lucide-react'
...
export default function PublicLayout() {
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col pb-16 md:pb-0">
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-card border-t flex items-center justify-around h-16 z-50 px-2 shadow-[0_-4px_10px_rgba(0,0,0,0.05)]">
        <Link to="/" className={`flex flex-col items-center justify-center w-full h-full space-y-1 ${isActive('/') ? 'text-primary' : 'text-muted-foreground'}`}>
          <Home className={`w-5 h-5 ${isActive('/') ? 'fill-primary/20' : ''}`} />
          <span className="text-[10px] font-medium">Beranda</span>
        </Link>
        <Link to="/lomba" className={`flex flex-col items-center justify-center w-full h-full space-y-1 ${isActive('/lomba') ? 'text-primary' : 'text-muted-foreground'}`}>
          <Trophy className={`w-5 h-5 ${isActive('/lomba') ? 'fill-primary/20' : ''}`} />
          <span className="text-[10px] font-medium">Lomba</span>
        </Link>
        <Link to="/informasi" className={`flex flex-col items-center justify-center w-full h-full space-y-1 ${isActive('/informasi') ? 'text-primary' : 'text-muted-foreground'}`}>
          <Info className={`w-5 h-5 ${isActive('/informasi') ? 'fill-primary/20' : ''}`} />
          <span className="text-[10px] font-medium">Informasi</span>
        </Link>
        <Link to="/transparansi" className={`flex flex-col items-center justify-center w-full h-full space-y-1 ${isActive('/transparansi') ? 'text-primary' : 'text-muted-foreground'}`}>
          <PieChart className={`w-5 h-5 ${isActive('/transparansi') ? 'fill-primary/20' : ''}`} />
          <span className="text-[10px] font-medium">Keuangan</span>
        </Link>
        {user ? (
          <Link to="/dashboard" className="flex flex-col items-center justify-center w-full h-full space-y-1 text-muted-foreground hover:text-primary">
            <User className="w-5 h-5" />
            <span className="text-[10px] font-medium">Akun</span>
          </Link>
        ) : (
          <Link to="/login" className="flex flex-col items-center justify-center w-full h-full space-y-1 text-muted-foreground hover:text-primary">
